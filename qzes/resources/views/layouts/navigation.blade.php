<nav x-data="{ open: false }" class="bck">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="flex items-center text-md font-medium text-gray-700 hover:text-gray-800 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
                    ...
                    <x-slot name="content">
                        ...
                        <form action="{{ route('user.show', Auth::user()->id) }}">
                            ...
                            <x-dropdown-link :href="route('user.show', Auth::user()->id)" onclick="event.preventDefault();
                                this.closest('form').submit();">
                                {{ __('Cuenta') }}
                            </x-dropdown-link>
                        </form>
                        @if (Auth::user()->role == 'admin')
                            <form action="{{ route('question.index') }}">
                                @csrf

                                <x-dropdown-link :href="route('question.index')" onclick="event.preventDefault();
                                this.closest('form').submit();">
                                    {{ __('Gestionar preguntas') }}
                                </x-dropdown-link>
                            </form>
                        @endif
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form action="{{ route('user.show', Auth::user()->id) }}">
                    ...
                    <x-responsive-nav-link :href="route('user.show', Auth::user()->id)" onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Cuenta') }}
                    </x-responsive-nav-link>
                </form>
                @if (Auth::user()->role == 'admin')
                    <form action="{{ route('question.index') }}">
                        @csrf
                        <x-responsive-nav-link :href="route('question.index')" onclick="event.preventDefault();
                                        this.closest('form').submit();">
                            {{ __('Gestionar preguntas') }}
                        </x-responsive-nav-link>
                    </form>
